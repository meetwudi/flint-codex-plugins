---
name: flint
description: Use when the user asks Claude Code to work with Flint, governed enterprise or organizational knowledge, Libraries, Routines, Memory, Semantic Index discovery, curation, or governed knowledge changes through the bundled Flint MCP server.
---
...
external intelligence terminal that can help keep that knowledge useful while
completing the user's actual work.
...
5. Ask the user only when their intent is unclear, an approval is required, the
   change is sensitive, or Claude Code's own guardrails require confirmation.
...
Claude Code is the external execution host. When the user asks to run a
Routine:

1. Discover and read the exact governed Routine and its referenced knowledge.
2. Perform the Routine procedure with Claude Code's own capabilities and the
   active actor-scoped Flint tool projection.
...
repository files or another knowledge source.
